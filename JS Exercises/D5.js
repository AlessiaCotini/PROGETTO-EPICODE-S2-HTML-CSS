/*
REGOLE
- Tutte le risposte devono essere scritte in JavaScript
- Puoi usare Google / StackOverflow ma solo quanto ritieni di aver bisogno di qualcosa che non è stato spiegato a lezione
- Puoi testare il tuo codice in un file separato, o de-commentando un esercizio alla volta
- Per visualizzare l'output, lancia il file HTML a cui è collegato e apri la console dagli strumenti di sviluppo del browser. 
- Utilizza dei console.log() per testare le tue variabili e/o i risultati delle espressioni che stai creando.
*/

/* ESERCIZIO 1
    Dato il seguente array, scrivi del codice per stampare ogni elemento dell'array in console.
*/
const pets = ["dog", "cat", "hamster", "redfish"];
console.log(pets[0]);
console.log(pets[1]);
console.log(pets[2]);
console.log(pets[3]);

/* ESERCIZIO 2
    Scrivi del codice per ordinare alfabeticamente gli elementi dell'array "pets".
*/
let ordinatedPets = pets.sort();
console.log(ordinatedPets);
/* ESERCIZIO 3
    Scrivi del codice per stampare nuovamente in console gli elementi dell'array "pets", questa volta in ordine invertito.
*/
let disorderPets = pets.reverse();
console.log(disorderPets);
/* ESERCIZIO 4
    Scrivi del codice per spostare il primo elemento dall'array "pets" in ultima posizione.
*/
let changedPets = pets[0];
pets[0] = pets[pets.length - 1];
pets[pets.length - 1] = changedPets;
console.log(pets);
/* ESERCIZIO 5
    Dato il seguente array di oggetti, scrivi del codice per aggiungere ad ognuno di essi una proprietà "licensePlate" 
    con valore a tua scelta.
*/
const cars = [
  {
    brand: "Ford",
    model: "Fiesta",
    color: "red",
    trims: ["titanium", "st", "active"],
  },
  {
    brand: "Peugeot",
    model: "208",
    color: "blue",
    trims: ["allure", "GT"],
  },
  {
    brand: "Volkswagen",
    model: "Polo",
    color: "black",
    trims: ["life", "style", "r-line"],
  },
];
cars.forEach((car, n) => {
  car.licensePlate = "Plate" + n;
});

console.log(cars);

/* ESERCIZIO 6
    Scrivi del codice per aggiungere un nuovo oggetto in ultima posizione nell'array "cars", rispettando la struttura
    degli altri elementi.
    Successivamente, rimuovi l'ultimo elemento della proprietà "trims" da ogni auto.
*/
cars.push({
  brand: "Fiat",
  model: "Panda",
  color: "coffe",
  trims: ["life", "style", "r-line"],
  licensePlate: "CV749NN",
});

for (let i = 0; i < cars.length; i++) {
  cars[i].trims.pop();
}
console.log(cars);
/* ESERCIZIO 7
    Scrivi del codice per salvare il primo elemento della proprietà "trims" 
    di ogni auto nel nuovo array "justTrims", sotto definito.
*/
const justTrims = [];
for (let i = 0; i < cars.length; i++) {
  justTrims.push(cars[i].trims[0]);
}
console.log(cars);
/* ESERCIZIO 8
    Cicla l'array "cars" e costruisci un if/else statament per mostrare due diversi messaggi in console. 
    Se la prima lettera della proprietà "color" ha valore "b", mostra in console "Fizz".
     Altrimenti, mostra in console "Buzz".
*/
for (let i = 0; i < cars.length; i++) {
  if (cars[i].color && cars[i].color[0].toLowerCase() === "b") {
    console.log("Fizz");
  } else {
    console.log("buzz");
  }
}
console.log(cars);
/* ESERCIZIO 9
    Utilizza un ciclo while per stampare in console i valori del seguente array numerico 
    fino al raggiungimento del numero 32.
*/
const numericArray = [
  6, 90, 45, 75, 84, 98, 35, 74, 31, 2, 8, 23, 100, 32, 66, 313, 321, 105,
];
let i = 0;

while (i < numericArray.length) {
  if (numericArray[i] === 66) {
    break;
  }
  console.log(numericArray[i]);
  i++;
}
/* ESERCIZIO 10
    Partendo dall'array fornito e utilizzando un costrutto switch, 
    genera un nuovo array composto dalle posizioni di ogni carattere all'interno
    dell'alfabeto italiano.
    es. [f, b, e] --> [6, 2, 5]
*/
const charactersArray = ["g", "n", "u", "z", "d"];
let newArrayNumber = [];
for (let i = 0; i < charactersArray.length; i++) {
  switch (charactersArray[i]) {
    case "a":
      newArrayNumber.push(1);
      break;
    case "b":
      newArrayNumber.push(2);
      break;
    case "c":
      newArrayNumber.push(3);
      break;
    case "d":
      newArrayNumber.push(4);
      break;
    case "e":
      newArrayNumber.push(5);
      break;
    case "f":
      newArrayNumber.push(6);
      break;
    case "g":
      newArrayNumber.push(7);
      break;
    case "h":
      newArrayNumber.push(8);
      break;
    case "i":
      newArrayNumber.push(9);
      break;
    case "l":
      newArrayNumber.push(10);
      break;
    case "m":
      newArrayNumber.push(11);
      break;
    case "n":
      newArrayNumber.push(12);
      break;
    case "o":
      newArrayNumber.push(13);
      break;
    case "p":
      newArrayNumber.push(14);
      break;
    case "q":
      newArrayNumber.push(15);
      break;
    case "r":
      newArrayNumber.push(16);
      break;
    case "s":
      newArrayNumber.push(17);
      break;
    case "t":
      newArrayNumber.push(18);
      break;
    case "u":
      newArrayNumber.push(19);
      break;
    case "v":
      newArrayNumber.push(20);
      break;
    case "z":
      newArrayNumber.push(21);
      break;
  }
}
console.log(newArrayNumber);
